...
from PySide6.QtWidgets import (
    ...
    QDateEdit, QCheckBox, QScrollArea, QSizePolicy, QGraphicsOpacityEffect
)
...
class ProjectView(QWidget):
    ...
    def init_ui(self):
        ...
        header = QLabel("🚀 Proje Yönetimi ve Planlama")
        ...
        # ==========================================
        left_panel = QFrame()
        left_panel.setStyleSheet("QFrame { background-color: #171412; border: 1px solid #292524; border-radius: 12px; }")
        ...
        btn_new.setStyleSheet("background-color: #38bdf8; color: #0c0a09; border-radius: 6px; padding: 10px; font-weight: bold; border: none;")
        btn_new.clicked.connect(self.new_project)
        left_lay.addWidget(btn_new)

        self.filter_category = QComboBox()
        self.filter_category.addItems(["Tüm Kategoriler", "Yarışma", "IoT / Donanım", "Yazılım", "Akademik"])
        ...
        self.filter_status.addItems(["Tüm Durumlar", "Planlanıyor", "Devam Ediyor", "Tamamlandı"])
        ...
        self.project_list.setStyleSheet("""
            ...
            QListWidget::item { padding: 12px; border-bottom: 1px solid #292524; color: #e4e4e7; border-radius: 6px; margin-bottom: 4px; background-color: #1c1917; }
            QListWidget::item:selected { background-color: #27272a; border-left: 4px solid #38bdf8; font-weight: bold; color: #38bdf8; }
        """)
        ...
        self.right_panel.setStyleSheet("QFrame { background-color: #171412; border: 1px solid #292524; border-radius: 12px; }")
        ...
        self.title_edit.setStyleSheet("background: transparent; border: none; font-size: 20px; font-weight: bold; color: #ffffff; border-bottom: 1px solid #3f3f46;")
        
        btn_save = QPushButton("💾 Kaydet")
        btn_save.setCursor(QCursor(Qt.PointingHandCursor))
        btn_save.setStyleSheet("background-color: #10b981; color: white; border-radius: 6px; padding: 8px 16px; font-weight: bold; border: none;")
        btn_save.clicked.connect(self.save_project)

        btn_delete = QPushButton("🗑 Sil")
        btn_delete.setCursor(QCursor(Qt.PointingHandCursor))
        btn_delete.setStyleSheet("background-color: #ef4444; color: white; border-radius: 6px; padding: 8px 16px; font-weight: bold; border: none;")
        ...
        self.cat_cb.addItems(["Yarışma", "IoT / Donanım", "Yazılım", "Akademik"])
        self.cat_cb.setStyleSheet(self.filter_category.styleSheet())
        
        self.status_cb = QComboBox()
        self.status_cb.addItems(["Planlanıyor", "Devam Ediyor", "Tamamlandı"])
        self.status_cb.setStyleSheet(self.filter_category.styleSheet())
        
        self.end_date_edit = QDateEdit()
        self.end_date_edit.setCalendarPopup(True)
        self.end_date_edit.setDate(QDate.currentDate())
        self.end_date_edit.setStyleSheet("background-color: #1c1917; color: white; padding: 6px; border-radius: 4px; border: 1px solid #3f3f46;")
        ...
        self.progress_bar.setStyleSheet("""
            QProgressBar { background-color: #27272a; border-radius: 6px; border: none; }
        ...
        task_lay.addWidget(QLabel("📋 Proje Aşamaları (Milestones)"))
        ...
        btn_add_task.setStyleSheet("background-color: #27272a; color: white; border-radius: 6px; padding: 6px 12px;")
        ...
        self.task_list.setStyleSheet("background: transparent; border: 1px solid #292524; border-radius: 6px; padding: 4px;")
        ...
        notes_lay.addWidget(QLabel("📝 Proje Defteri (Sistem Mimarisi, Notlar, Loglar)"))
        ...
        self.notes_edit.setStyleSheet("background-color: #1c1917; border: 1px solid #292524; border-radius: 8px; padding: 12px; color: #f4f4f5; font-size: 14px; line-height: 1.5;")
        ...
        # Bu ayar panellerin ekranın tamamına yayılmasını sağlar
    ...
    def load_tasks(self):
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT id, title, is_completed FROM project_tasks WHERE project_id = ?", (self.active_project_id,))
            for r in cur.fetchall():
