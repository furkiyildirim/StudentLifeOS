...
# =========================================================================
class InteractiveCircularTimer(QWidget):
    ...
    def __init__(self):
        ...
        self.color = QColor("#f43f5e") 
        self.is_dragging = False
        self.last_tick_min = -1
        self.setCursor(QCursor(Qt.PointingHandCursor))

    def update_state(self, time_left: int, total_time: int, is_running: bool, color_hex: str = None):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            dist = self.get_distance_from_center(event.position())
            if dist < (self.width() / 2) - 40:
                self.toggleRequested.emit()
            else:
                if not self.is_running:
                    self.is_dragging = True
                    self.set_time_from_pos(event.position())
    ...
    def set_time_from_pos(self, pos):
        angle = self.get_angle_from_pos(pos)
        mins = int((angle / 360.0) * 60)
        if mins == 0:
            if angle > 350: mins = 60
            else: mins = 1
                
        if mins != self.last_tick_min:
            self.last_tick_min = mins
            self.timeSet.emit(mins)
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(QPen(QColor("#3f3f46"), 2, Qt.SolidLine, Qt.RoundCap))
        for i in range(60):
            angle = math.radians(i * 6 - 90)
            is_five_min = (i % 5 == 0)
            inner_r = radius - (12 if is_five_min else 6)
        ...
        ratio = self.time_left / 3600.0 
        if ratio > 1.0: ratio = 1.0

        pen_fg = QPen(self.color, 6)
        ...
        knob_angle = math.radians(ratio * 360 - 90)
        knob_x = cx + radius * math.cos(knob_angle)
        knob_y = cy + radius * math.sin(knob_angle)

        knob_size = 12 if self.is_dragging else 8
        painter.setBrush(self.color)
        painter.setPen(QPen(QColor("#ffffff"), 2))
        painter.drawEllipse(QRectF(knob_x - knob_size, knob_y - knob_size, knob_size * 2, knob_size * 2))

        mins, secs = divmod(self.time_left, 60)
        painter.setPen(QColor("#ffffff"))
        painter.setFont(QFont("Inter", 48, QFont.Black))
        painter.drawText(rect, Qt.AlignCenter, f"{mins:02d}:{secs:02d}")

        status_str = "ÇALIŞIYOR (DURDUR)" if self.is_running else "BAŞLATMAK İÇİN TIKLA"
        if not self.is_running and self.time_left < self.total_time and self.time_left > 0:
            status_str = "DURAKLATILDI"
            
        painter.setFont(QFont("Inter", 9, QFont.Bold))
        painter.setPen(QColor("#10b981") if self.is_running else QColor("#a1a1aa"))
        painter.drawText(QRectF(rect.x(), rect.y() + 70, rect.width(), rect.height()), Qt.AlignCenter, status_str)
...
class MusicView(QWidget):
    ...
    def create_local_music_tab(self):
        ...
        # --- Üst Kısım: Görsel Çalma Listesi Seçici ---
        ...
        # Çalma Listesi Kartları (Yatay Kaydırmalı)
    ...
    # =========================================================================
    # YENİ EKLENEN: GÖRSEL PLAYLIST İŞLEMLERİ VE ŞARKI TRANSFERİ
    # =========================================================================
    ...
    def add_track_to_ui(self, file_path):
        for i in range(self.playlist_widget.count()):
            if self.playlist_widget.item(i).data(Qt.UserRole) == file_path:
                return 
    ...
    def add_local_music(self):
        ...
        # Güncellenen kapağı çekmek için
        self.load_playlists()
    ...
    def create_focus_tab(self):
        ...
        btn_focus = QPushButton("Odak (25 Dk)")
        btn_focus.setCursor(QCursor(Qt.PointingHandCursor))
        btn_focus.setStyleSheet("background-color: rgba(244, 63, 94, 0.2); color: #f43f5e; border: 1px solid #f43f5e; border-radius: 6px; padding: 6px 12px; font-weight: bold;")
        btn_focus.clicked.connect(lambda: self.set_preset(25, "#f43f5e"))

        btn_short = QPushButton("Mola (5 Dk)")
        btn_short.setCursor(QCursor(Qt.PointingHandCursor))
        btn_short.setStyleSheet("background-color: rgba(56, 189, 248, 0.2); color: #38bdf8; border: 1px solid #38bdf8; border-radius: 6px; padding: 6px 12px; font-weight: bold;")
        btn_short.clicked.connect(lambda: self.set_preset(5, "#38bdf8"))

        btn_long = QPushButton("Uzun Mola (15 Dk)")
        btn_long.setCursor(QCursor(Qt.PointingHandCursor))
        btn_long.setStyleSheet("background-color: rgba(16, 185, 129, 0.2); color: #10b981; border: 1px solid #10b981; border-radius: 6px; padding: 6px 12px; font-weight: bold;")
        btn_long.clicked.connect(lambda: self.set_preset(15, "#10b981"))

        mode_row.addWidget(btn_focus)
        mode_row.addWidget(btn_short)
        mode_row.addWidget(btn_long)
        lay.addLayout(mode_row)

        hint = QLabel("💡 İpucu: Süreyi ayarlamak için ucundaki beyaz noktayı sürükleyin. Başlatmak için ortaya tıklayın.")
        hint.setStyleSheet("color: #a1a1aa; font-size: 12px;")
        hint.setAlignment(Qt.AlignCenter)
        lay.addWidget(hint)

        self.circular_timer = InteractiveCircularTimer()
        self.circular_timer.toggleRequested.connect(self.toggle_timer)
        self.circular_timer.timeSet.connect(self.set_custom_time_from_dial)
        ...
        lay.addWidget(self.circular_timer, alignment=Qt.AlignCenter)

        btn_reset = QPushButton("🔄 Sayacı Sıfırla")
        btn_reset.setFixedWidth(140)
        btn_reset.setCursor(QCursor(Qt.PointingHandCursor))
        btn_reset.setStyleSheet("background-color: transparent; border: 1px solid #3f3f46; color: #a1a1aa; border-radius: 6px; padding: 6px; font-weight: bold;")
    ...
    def timer_tick(self):
        time_left = self.circular_timer.time_left
        ...
        if time_left > 0:
            ...
            if self.main_window and hasattr(self.main_window, 'send_tray_notification'):
                self.main_window.send_tray_notification(
                    "Süre Doldu! ⏳",
                    "Odak sayacınız tamamlandı! Harika iş çıkardın, mola verebilirsin.",
                    color="#f43f5e",
